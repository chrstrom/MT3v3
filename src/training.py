...
def copy_code_dependencies(logger, args):
    ...
    os.makedirs(code_used_folder)

    shutil.copy(args.task_params, os.path.join(code_used_folder, "task_params.yaml"))
    shutil.copy(args.model_params, os.path.join(code_used_folder, "model_params.yaml"))
    ...
    if args.continue_training_from is not None:
        try:
            shutil.copytree(
                os.path.join(args.continue_training_from, "code_used"),
                os.path.join(logger.log_path, "code_from_previous_training"),
            )
        except FileNotFoundError:
            print(
                f"Path specified to continue training from does not exist: {args.continue_training_from}"
            )
            exit()
...
def load_loss_models(params, eval_params):
    ...
    return mot_loss, mot_loss_eval, contrastive_loss, false_loss
...
if __name__ == '__main__':
    ...
    eval_params.data_generation.seed = seed + 1 # make sure we don't evaluate with same seed as final evaluation after training
    torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)
    np.random.seed(seed)

    logger = Logger(
        log_path=f'.{os.path.sep}results{os.path.sep}{time.strftime("%Y-%m-%d_%H%M%S")}',
        save_output=False,
    ...
    copy_code_dependencies(logger, args)
    ...
    # Set up data generation, model and losses

    print(f"Starting data generator with {params.training.batch_size} batches")
    data_generator = DataGenerator(params)
    
    model, optimizer, scheduler = load_mott_model(args, params, eval_params)
    
    mot_loss, mot_loss_eval, contrastive_loss, false_loss = load_loss_models(params, eval_params)
    ...
    if params.debug.enable_plot or params.debug.save_plot_figs:
        ...
        percent_ax.grid("on")
        (matched_median_cert_line,) = percent_ax.plot(
            [1], "C0", label="Matched median certainty"
        )
        (unmatched_median_cert_line,) = percent_ax.plot(
            [1], "C3", label="Unmatched median certainty"
        )
        ...
        if params.loss.contrastive_classifier:
            (
                contrastive_loss_fig,
                contrastive_loss_ax,
                contrastive_loss_line,
            ) = get_constrastive_ax()
        ...
        (
            state_uncertainties_fig,
            state_uncertainties_ax,
            state_uncertainties_lines,
    ...
    for i_gradient_step in range(params.training.n_gradient_steps):
        ...
        for i_split_step in range(n_splits):
            try:
                ...
                prediction, intermediate_predictions, encoder_prediction, aux_classifications, _ = \
                    model.forward(batch)
                loss_dict, indices = mot_loss.forward(labels, prediction, intermediate_predictions, encoder_prediction,
                ...
                if params.loss.type == 'both':
                    ...
                    logs = update_logs(logs, 'detr', loss_dict['detr_state'].item() + loss_dict['detr_logits'].item())
                    logs = update_logs(logs, 'gospa', loss_dict['gospa_state'].item())
                    logs = update_logs(logs, 'last_layer_losses', logs['detr'] + logs['gospa']*gospa_weight)

                else:
                    total_loss = sum(loss_dict[k] for k in loss_dict.keys())
                    if params.loss.type == 'dhn':
                        logs = update_logs(logs, params.loss.type, total_loss.item())
                        logs = update_logs(logs, 'last_layer_losses', total_loss.item())
                    else:
                ...
                if params.loss.false_classifier:
                    ...
                    logs = update_logs(logs, 'false_loss', f_loss.item())
                
                if params.arch.type == 'kreacher':
                    f_loss = false_loss(aux_classifications['false_filter'], unique_ids)
                    total_loss = total_loss + f_loss * 25
                    logs = update_logs(logs, 'false_filter_loss', f_loss.item()* 25)
        ...
        if (params.debug.enable_plot and i_gradient_step % params.debug.plot_interval == 0) or \
            ...
            output_ax.set_xlim([-params.data_generation.field_of_view.max_range, params.data_generation.field_of_view.max_range])
            ...
            if params.debug.save_plot_figs and i_gradient_step % params.debug.save_plot_figs_interval == 0:
                filename = f"gradient_step{i_gradient_step}.svg"
