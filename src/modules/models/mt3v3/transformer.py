...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, return_intermediate=False, debug=False, with_state_refine=False):
        super().__init__()
    ...
    def forward(self,
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            p_deltas = self.obj_classifier[layer_idx](self.norm(object_queries).permute(1, 0, 2))
            if predicted_logits_deltas is None:
                predicted_logits_deltas = p_deltas.sigmoid()
            else:
                predicted_logits_deltas = (predicted_logits_deltas + p_deltas).sigmoid()
            ...
            # Save intermediate results for auxiliary loss if needed
            if self.return_intermediate:
                ...
                intermediate_logits.append(p_deltas)
        ...
        if self.return_intermediate:
            ...
            return intermediate_state_predictions_normalized, intermediate_uncertainties, predicted_logits_deltas, intermediate_logits, debug_dict
        else:
