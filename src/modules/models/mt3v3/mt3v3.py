...
from modules.position_encoder import Time2Vec
from modules.mlp import MLP
from modules.models.mt3v3.transformer import TransformerEncoder, TransformerDecoder, PreProccessor, TransformerEncoderLayer, TransformerDecoderLayer
...
class MOTT(nn.Module):
    def __init__(self, params):
        ...
        self.output_scaling_factor = [params.data_generation.field_of_view.max_range * 4,
                                      params.data_generation.field_of_view.max_range * 4]
        ...
        self.uncertainty_predictor = MLP(params.arch.d_model,
                                         hidden_dim=params.arch.d_prediction_hidden,
                                         output_dim=2*self.prediction_space_dimensions,
                                         num_layers=params.arch.n_prediction_layers,
        ...
        if self.params.loss.contrastive_classifier:
            self.contrastive_classifier = ContrastiveClassifier(params.arch.d_model, params.training.device)
    ...
    def get_two_stage_proposals(self, measurement_batch, embeddings):
        ...
        xs = measurements[:, :, 0] * (measurements[:, :, 1].cos())
        ys = measurements[:, :, 0] * (measurements[:, :, 1].sin())
    ...
    def forward(self, measurements: NestedTensor):
        ...
        intermediate_state_predictions_normalized, intermediate_uncertainties, logits, intermediate_logits, debug_dict = result
