...
class Logger:
    def __init__(self, log_path, save_output=True, buffer_size=1):
        ...
        if save_output:
            sys.stdout = FileAndStream(
                self.log_path + os.path.sep + "out.log", sys.stdout
            )
            sys.stderr = FileAndStream(
                self.log_path + os.path.sep + "out_err.log", sys.stderr
            )
    ...
    def log_scalar(self, name, value, t, flush_now=False):
        ...
        if (self.buffer[name]["n"] >= self.buffer_size) or flush_now:
            filename = self.log_path + os.path.sep + name + ".txt"
        ...
        for module in modules:
            if hasattr(module, "__file__"):
                ...
                if filename is not None:
                    ...
                    if (
                        project_root_path in filename
                        or filename == sys.argv[0]
                        or file_in_additional_paths
                    ):
                        destination = (
                            self.log_path
                            + "code_used/"
                            + filename.replace(project_root_path, "")
                        )
    ...
    try:
        ...
        if not os.path.isdir(log_folder):
            ...
            raise FileNotFoundError(
                f"Could not find file {filename} inside {log_folder}"
            )
        else:
            raise fne
