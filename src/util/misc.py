...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor]):
    ...
    if tensor_list[0].ndim == 2:
        ...
        #     pad_meas[: meas.shape[0], : meas.shape[1]].copy_(meas)
        #     m[: meas.shape[1]] = False
    else:
...
class Prediction:
    ...
    def get_predictions(self):
        pos = self.positions.squeeze().detach().cpu().numpy()
        vel = self.velocities.squeeze().detach().cpu().numpy()
        p = self.logits.sigmoid().squeeze().detach().cpu().numpy().reshape(-1, 1)

        return np.concatenate((pos, vel, p), axis=1)
    
    def get_uncertainties(self):
        return self.uncertainties.squeeze().detach().cpu().numpy()

    def __repr__(self):
        pos_strs = [
            f"pos: [{pos[0]:.4f}, {pos[1]:.4f}]" for pos in self.positions.squeeze()
        ]
        vel_strs = [
            f"vel: [{vel[0]:.4f}, {vel[1]:.4f}]" for vel in self.velocities.squeeze()
        ]
        p_strs = [
            f"p: {p:.4f}" for p in self.logits.sigmoid().squeeze()
        ]  # Existence probabilities
        pred_str = "\n".join(
            [
                f"{pos_str} \t {vel_str}  \t {p_str}"
                for pos_str, vel_str, p_str in zip(pos_strs, vel_strs, p_strs)
            ]
        )
