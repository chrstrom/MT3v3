...
def output_truth_plot(ax, prediction, labels, matched_idx, batch, params, training_example_to_plot=0):
    ...
    ax.scatter(measurements_cpu[:, 0]*np.cos(measurements_cpu[:, 1]),
    ...
    for i_r in range(len(grid_r) - 1):
        for i_theta in range(len(grid_theta) - 1):
            ...
            rect = patches.Rectangle((sector[2], sector[0]),
                                     ...
                                     edgecolor='none')
            ax.add_patch(rect)
            ax.text(sector[2]+delta_theta/2, sector[0]+delta_r/2, f"{values[i_r, i_theta]:.1f}", horizontalalignment='center', verticalalignment='center')
    ax.autoscale_view()
    ax.set_thetamin(-90)
    ax.set_thetamax(90)
    ax.bar(0, 1).remove()
